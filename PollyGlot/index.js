...
navLinks.forEach(link => {
    if (link.getAttribute('href') === currentPage || 
        (currentPage.includes('index') && link.getAttribute('href').includes('index')) ||
...
async function fetchStockData() {
    ...
    try {
        ...
        fetchReport(stockData.join('\n\n')) // Same as before — just pass the raw data string
    } catch (err) {
...
// EXACT SAME STRUCTURE AS YOUR ORIGINAL OPENAI CODE
async function fetchReport(data) {
    ...
    try {
        // Only this tiny block changed — everything else is identical
        const genAI = new GoogleGenerativeAI(import.meta.env.VITE_GEMINI_API_KEY)
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash", temperature: 1.1 })

        // Convert messages exactly like your working example
